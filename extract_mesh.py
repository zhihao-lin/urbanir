...
dense_xyz = torch.stack(torch.meshgrid(
        torch.linspace(x_min, x_max, 256),
        torch.linspace(y_min, y_max, 256),
        torch.linspace(z_min, z_max, 128),
    ), -1).cuda()
...
convert_samples_to_ply(density.detach().cpu(), os.path.join(f'results/{hparams.dataset_name}/{hparams.exp_name}', 'mesh.ply'), bbox=bbox.cpu(), level=10)
os.system('cp extract_mesh.py {}'.format(os.path.join(f'results/{hparams.dataset_name}/{hparams.exp_name}')))