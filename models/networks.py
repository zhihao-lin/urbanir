...
class NGP(nn.Module):
    def __init__(self, scale, rgb_act='Sigmoid', embed_a=False, embed_a_len=12, classes=7):
        super().__init__()
        ...
        self.sigma_net = nn.Sequential(
            nn.Linear(self.sigma_encoder.n_output_dims, 128),
            nn.Softplus(),
            nn.Linear(128, 1)
        )
        ...
        app_enc_dim = self.appearance_encoder.n_output_dims # + self.dir_encoder.n_output_dims
        dir_enc_dim = self.dir_encoder.n_output_dims

        self.rgb_net = \
            tcnn.Network(
                n_input_dims=app_enc_dim+dir_enc_dim+embed_a_len if embed_a else app_enc_dim+dir_enc_dim ,
                n_output_dims=3,
                network_config={
                    "otype": "CutlassMLP",
                    "activation": "ReLU",
        ...
        self.albedo_net = \
            tcnn.Network(
                n_input_dims=app_enc_dim+embed_a_len if embed_a else app_enc_dim,
                n_output_dims=3,
                network_config={
                    "otype": "CutlassMLP",
                    "activation": "ReLU",
        ...
        self.norm_pred_header = tcnn.Network(
                    n_input_dims=app_enc_dim, n_output_dims=3,
                    network_config={
                        "otype": "CutlassMLP",
                        "activation": "ReLU",
        ...
        self.semantic_header = tcnn.Network(
                    n_input_dims=app_enc_dim, n_output_dims=classes,
                    network_config={
                        "otype": "CutlassMLP",
                        "activation": "ReLU",
                        "output_activation": "None",
        ...
        self.visibility_header = tcnn.Network(
                    n_input_dims=app_enc_dim, n_output_dims=2,
                    network_config={
                        "otype": "CutlassMLP",
                        "activation": "ReLU",
                        "output_activation": "None",
        ...
        self.visibility_act = nn.Softmax(dim=-1)
    ...
    def density(self, x, return_feat=False, grad=True, grad_feat=True):
        ...
        with torch.set_grad_enabled(grad):
            h = self.sigma_encoder(x)
            h = self.sigma_net(h)
            sigmas = self.sigma_act(h[:, 0])
        if return_feat: 
            with torch.set_grad_enabled(grad_feat):
                feat_a = self.appearance_encoder(x)
                return sigmas, feat_a
        return sigmas

    @torch.enable_grad()
    def grad(self, x):
        x = x.requires_grad_(True)
        sigmas, feat_a = self.density(x, return_feat=True)
    ...
    def forward(self, x, d, **kwargs):
        ...
        if self.embed_a:
            ...
            if embed_a.size(0) < feat_a.size(0):
                repeat = int(feat_a.size(0)/embed_a.size(0))
                embed_a = torch.repeat_interleave(embed_a, repeat, 0)
            rgb = self.rgb_net(torch.cat([feat_a, d, embed_a], 1))
            albedo = self.albedo_net(torch.cat([feat_a, embed_a], 1))
        else:
            rgb = self.rgb_net(torch.cat([feat_a, d], 1))
            albedo = self.albedo_net(feat_a)
            
        return sigmas, rgb, albedo, normals_raw, normals_pred, semantic, visibility
    ...
    def forward_test(self, x, d, **kwargs):
        ...
        Outputs:
            ...
            rgb: (N, 3)
        """
        # sigmas, feat_a = self.density(x, return_feat=True, grad=False, grad_feat=False)
        sigmas, feat_a, grads = self.grad(x)
        normals_raw = -F.normalize(grads, p=2, dim=-1, eps=1e-6)
        ...
        return sigmas, rgb, albedo, normals_pred, normals_raw, semantic, visibility
    ...
    def sample_uniform_and_occupied_cells(self, M, density_threshold):
        ...
        for c in range(self.cascades):
            ...
            # concatenate
            if (indices1.numel() == 0 or indices2.numel() == 0 or coords1.numel() == 0 or coords2.numel() == 0):
                print('indices1:', indices1.size())
                print('indices2:', indices2.size())
                print('coords1:',  coords1.size())
                print('coords2:',  coords2.size())
                raise Exception('Contains invalid tensor(s)')
...
# write a unit test for class NGP
def test_NGP():
    ...
    xyzs = torch.randn(sample_n, 3).cuda()
    dirs = torch.randn(sample_n, 3).cuda()
    # test the forward for training
    sigmas, rgb, albedo, normals_raw, normals_pred, semantic, visibility = ngp(xyzs, dirs, **kwargs)
    print('=== forward training ===')
    print('albedo', albedo.shape)

    # test the forward for inference
    sigmas, rgb, albedo, normals_pred, normals_raw, semantic, visibility = ngp.forward_test(xyzs, dirs, **kwargs)
