...
class VolumeRenderer(torch.autograd.Function):
    ...
    @staticmethod
    # @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, rgbs, albedos, normals_pred, sems, viss, deltas, ts, rays_a, T_threshold, classes):
        total_samples, opacity, depth, rgb, albedo, normal_pred, sem, vis, ws = \
            vren.composite_train_fw(sigmas, rgbs, albedos, normals_pred, sems, viss, deltas, ts,
                                    rays_a, T_threshold, classes)
        ctx.save_for_backward(sigmas, rgbs, albedos, viss, deltas, ts, rays_a,
                              opacity, depth, rgb, albedo, vis, ws)
        ctx.T_threshold = T_threshold
        ctx.classes = classes
        return total_samples.sum(), opacity, depth, rgb, albedo, normal_pred, sem, vis, ws

    @staticmethod
    # @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drgb, dL_dalbedo, dL_dnormal_pred, dL_dsem, dL_dvis, dL_dws):
        sigmas, rgbs, albedos, viss, deltas, ts, rays_a, \
        opacity, depth, rgb, albedo, vis, ws = ctx.saved_tensors
        dL_dsigmas, dL_drgbs, dL_dalbedos, dL_dnormals_pred, dL_dsems, dL_dviss = \
            vren.composite_train_bw(dL_dopacity, dL_ddepth,
                                    dL_drgb, dL_dalbedo, dL_dnormal_pred, dL_dsem, dL_dvis, dL_dws,
                                    sigmas, rgbs, albedos, viss, ws, deltas, ts,
                                    rays_a,
                                    opacity, depth, rgb, albedo, vis,
                                    ctx.T_threshold, ctx.classes)
        
        return dL_dsigmas, dL_drgbs, dL_dalbedos, dL_dnormals_pred, dL_dsems, dL_dviss, None, None, None, None, None

class VolumeRendererVisibility(torch.autograd.Function):
    """
    Calculate visibility as transmittance by volume rendering
    Used in training only
    ...
    Inputs:
        ...
        rays_a: (N_rays, 3) ray_idx, start_idx, N_samples
                meaning each entry corresponds to the @ray_idx th ray,
                whose samples are [start_idx:start_idx+N_samples]
        light_dist: (N_rays) distance from ray origin to light source (sky)
        T_threshold: float, stop the ray if the transmittance is below it
    ...
    @staticmethod
    # @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, deltas, ts, rays_a, light_dist, T_threshold):
        visibility, ws = \
            vren.visibility_train_fw(sigmas, deltas, ts,
                                    rays_a, light_dist, T_threshold)
        ctx.save_for_backward(sigmas, deltas, ts, rays_a, visibility, light_dist, ws)
        ctx.T_threshold = T_threshold
        return visibility, ws

    @staticmethod
    # @custom_bwd
    def backward(ctx, dL_dvisibility, dL_dws):
        sigmas, deltas, ts, rays_a, visibility, light_dist, ws = ctx.saved_tensors

        dL_dsigmas = vren.visibility_train_bw(
            dL_dvisibility, dL_dws, sigmas, ws, deltas, ts, 
            rays_a, visibility, light_dist, ctx.T_threshold)
        
        return dL_dsigmas, None, None, None, None, None
